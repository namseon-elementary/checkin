var SPREADSHEET_ID = '1wk3WHhfNhjfxh1UEOxdp-aA22549TrPU6xsvyrWn11Y';
...
function doGet(e) {
  ...
  return out({ status: 'ok', message: '남선초 running' });
}

// 등교체크 탭에서 특정 학생의 제출 횟수를 집계
function getCheckinCountForStudent(ss, name) {
  if (!name) return 0;
  var sheet = ss.getSheetByName('등교체크');
  if (!sheet) return 0;
...
var SECTIONS = [
  { key:'morning', tab:'등교체크', label:'🌅 등교 체크', titleBg:'#FF6B35', headerBg:'#FFE8DF', bandBg:'#FFF8F4' },
  { key:'evening', tab:'하교체크', label:'🌙 하교 체크', titleBg:'#5BAD8A', headerBg:'#E0F7F5', bandBg:'#F4FBFA' },
...
function onOpen() {
  try {
    SpreadsheetApp.getUi().createMenu('🦊 남선초')
...
function setupStudentSheet() {
  ...
  // 현재 학생 목록 (남선초 담당자가 학생 관리 탭에서 직접 추가)
  var names = [];
